...
describe HtmlExpectationsHook do
  ...
  describe 'body DeclaresStyle:' do
    let(:code) { "<head> <style> p, h2 {color: blue; font-size: 4px;} div.cuadrado circulo {background: red} div.withBorder { border: 12px solid black; } div.withSpecificFont { font-family: Georgia , Serif} </style> </head>" }
    let(:expectations) { [
